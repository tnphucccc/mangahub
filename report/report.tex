\documentclass[12pt,a4paper]{report}% --- Packages ---\usepackage[utf8]{inputenc}\usepackage[T1]{fontenc}\usepackage{geometry}\geometry{margin=1in}\usepackage{graphicx}\usepackage{booktabs} % For professional tables\usepackage{hyperref} % For clickable links\usepackage{titlesec} % For custom chapter titles\usepackage{listings} % For code snippets\usepackage{xcolor}% --- Settings ---\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=cyan}\lstset{basicstyle=\ttfamily\small, breaklines=true, frame=single}% --- Document Start ---\begin{document}% --- Title Page ---\begin{titlepage}    \centering    \vspace*{1cm}    {\huge \textbf{Project Title}} \\    \vspace{1.5cm}    {\large \textbf{Author Name}} \\    \vfill    A report submitted in partial fulfillment of the requirements for... \\    \vspace{0.8cm}    Department of Computer Science \\    University Name \\    \today\end{titlepage}% --- Front Matter ---\pagenumbering{roman}\tableofcontents\listoffigures\listoftables\chapter*{Contribution Table}\addcontentsline{toc}{chapter}{Contribution Table}\begin{table}[h]    \centering    \begin{tabular}{|l|p{10cm}|}        \hline        \textbf{Member Name} & \textbf{Contribution} \\ \hline        Student 1 & Implementation of TCP Sync Server and Chapter 4 writing. \\ \hline        Student 2 & System Architecture design and Database schema. \\ \hline    \end{tabular}    \caption{Project Contribution Summary}\end{table}\chapter*{Abstract}\addcontentsline{toc}{chapter}{Abstract}
...
\newpage\pagenumbering{arabic}% --- Chapter 1 ---\chapter{Introduction}
...
\begin{description}
    \item[Go (Golang) 1.19+] Serves as the primary backend language, chosen for its exceptional concurrency model with goroutines and channels.
    \item[TypeScript 5.0+] Used for frontend development, providing type safety and improved developer experience over JavaScript.
    \item[Gin Framework] A high-performance HTTP web framework for building RESTful APIs in Go.
    \item[Gorilla WebSocket] A battle-tested Go library for implementing WebSocket servers.
    \item[gRPC-Go] The official Go implementation of gRPC for high-performance RPC.
    \item[Next.js 14] A React framework for building server-side rendered and static web applications.
    \item[React 18] A component-based UI library for building user interfaces.
    \item[Tailwind CSS 3] A utility-first CSS framework for rapid UI development.
    \item[SQLite3] An embedded relational database for simple, zero-configuration data storage.
    \item[Git/GitHub] Distributed version control and collaboration platform.
    \item[Visual Studio Code] The primary IDE with extensions for Go, ESLint, and Prettier.
    \item[Docker] Containerization platform for packaging and deploying applications.
\end{description}% --- Chapter 2 ---\chapter{Literature Review}
