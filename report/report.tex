...
The HTTP server is built using the Gin web framework (v1.11.0) and serves as the main entry point for client interactions. It runs on port 8080 and handles all RESTful API operations for user authentication, manga catalog management, and library operations.

\subsection{Server Configuration and Initialization}
The server is started from \texttt{cmd/api-server/main.go}, which initializes the database connection, sets up routing with middleware, and starts both HTTP and WebSocket listeners. The implementation follows a three-layer architecture:

\begin{lstlisting}[language=go, caption=HTTP Server Initialization, basicstyle=\ttfamily\footnotesize]
func main() {
    config := config.LoadConfig("./configs/dev.yaml")
    db := database.InitDB(config.Database.Path)
    jwtManager := auth.NewJWTManager(config.JWT.Secret, config.JWT.ExpiryDays)

    router := gin.Default()
    router.Use(middleware.CORSMiddleware())

    setupRoutes(router, db, jwtManager)
    ...
    log.Println("HTTP server listening on port", config.Server.HTTPPort)
    server.ListenAndServe()
}
\end{lstlisting}

\subsection{Route Organization}
Routes are organized hierarchically with appropriate middleware applied at different levels. The API follows RESTful conventions with versioning (\texttt{/api/v1}):
...
\begin{tabular}{|l|l|l|}
\hline
\textbf{Endpoint} & \textbf{Method} & \textbf{Authentication} \\ \hline
/api/v1/auth/register & POST & No \\ \hline
/api/v1/auth/login & POST & No \\ \hline
/api/v1/manga & GET & No \\ \hline
/api/v1/manga/:id & GET & No \\ \hline
/api/v1/users/library & GET & Required \\ \hline
/api/v1/users/library & POST & Required \\ \hline
/api/v1/users/progress/:manga\_id & GET & Required \\ \hline
/api/v1/users/progress/:manga\_id & PUT & Required \\ \hline
/api/v1/users/stats & GET & Required \\ \hline
/health & GET & No \\ \hline
\end{tabular}
\caption{HTTP API Endpoints Summary}
\label{tab:http-endpoints}
\end{table}

\subsection{Authentication Implementation}
JWT-based authentication is implemented using the \texttt{golang-jwt/jwt/v4} library with HMAC-SHA256 signing. Passwords are hashed using bcrypt with a cost factor of 10:

\begin{lstlisting}[language=go, caption=JWT Token Generation, basicstyle=\ttfamily\footnotesize]
    ...
    claims := &Claims{
        ...
        RegisteredClaims: jwt.RegisteredClaims{
            ExpiresAt: jwt.NewNumericDate(time.Now().Add(
                time.Duration(j.expiryDays) * 24 * time.Hour)),
...
\subsection{Handler-Service-Repository Pattern}
The implementation follows clean architecture principles with clear separation of concerns:

\begin{lstlisting}[language=go, caption=Example: Search Manga Flow, basicstyle=\ttfamily\footnotesize]
// Handler Layer (internal/manga/handler.go)
func (h *Handler) SearchManga(c *gin.Context) {
    ...
    mangas, err := h.service.SearchManga(query)
    if err != nil {
        c.JSON(500, gin.H{"error": err.Error()})
        return
    }

    c.JSON(200, gin.H{"success": true, "data": mangas})
}

// Service Layer (internal/manga/service.go)
func (s *Service) SearchManga(query models.MangaSearchQuery) ([]models.Manga, error) {
    return s.repo.FindByQuery(query)
}

// Repository Layer (internal/manga/repository.go)
func (r *Repository) FindByQuery(query models.MangaSearchQuery) ([]models.Manga, error) {
    sqlQuery := "SELECT * FROM manga WHERE 1=1"
    args := []interface{}{}
...
The TCP server provides real-time synchronization of reading progress across multiple devices using persistent connections. It runs on port 9090 and implements a custom JSON-based messaging protocol over TCP.

\subsection{Server Architecture}
The server is launched from \texttt{cmd/tcp-server/main.go} and uses Go's \texttt{net} package for low-level TCP operations:

\begin{lstlisting}[language=go, caption=TCP Server Structure, basicstyle=\ttfamily\footnotesize]
type Server struct {
    Port       string
    listener   net.Listener
    clients    map[string]*Client        // clientID -> Client
    userIndex  map[string][]*Client      // userID -> []*Client
...
\subsection{Connection Handling and Concurrency}
The server accepts multiple concurrent connections, with each connection handled in a dedicated goroutine:

\begin{lstlisting}[language=go, caption=TCP Server Main Loop, basicstyle=\ttfamily\footnotesize]
...
\subsection{JSON-Based Messaging Protocol}
Messages are newline-delimited JSON with type fields:
...
\hline
\textbf{Message Type} & \textbf{Description} \\ \hline
auth & Client sends JWT token for authentication \\ \hline
auth\_success / auth\_failed & Server authentication response \\ \hline
ping / pong & Heartbeat mechanism \\ \hline
progress & Client sends reading progress update \\ \hline
broadcast & Server broadcasts progress to user's devices \\ \hline
\end{tabular}
\caption{TCP Protocol Message Types}
\end{table}
...
The UDP server broadcasts chapter release notifications using a connectionless protocol. It runs on port 9091 and implements a registration-based notification system with automatic client cleanup.

\subsection{Server Architecture and Client Management}
Started from \texttt{cmd/udp-server/main.go}, the server maintains a registry of subscribed clients with automatic staleness detection:

\begin{lstlisting}[language=go, caption=UDP Server Structure, basicstyle=\ttfamily\footnotesize]
type Server struct {
    Port       string
    conn       *net.UDPConn
    ...
    mu         sync.RWMutex
    bufferSize int  // 2KB UDP receive buffer
}
...
The server implements automatic client management:
\begin{enumerate}
    \item \textbf{Registration}: Clients send \texttt{register} messages
    \item \textbf{Heartbeat}: Periodic \texttt{ping} messages (30s intervals)
    \item \textbf{Cleanup}: Stale clients removed after 5 minutes of inactivity
\end{enumerate}

\subsection{Fire-and-Forget Broadcasting}
Notifications are broadcast without acknowledgment, demonstrating UDP's best-effort delivery:

\begin{lstlisting}[language=go, caption=UDP Notification Broadcast, basicstyle=\ttfamily\footnotesize]
func (s *Server) BroadcastNotification(notif models.UDPNotification) {
    ...
    for _, client := range clients {
        go func(addr *net.UDPAddr) {  // Non-blocking
...
The WebSocket server enables real-time bidirectional communication for chat functionality. It runs on port 9093 and supports multiple chat rooms with user presence detection.

\subsection{Hub-and-Client Architecture}
The implementation follows the standard hub-and-client pattern recommended by Gorilla WebSocket:

\begin{lstlisting}[language=go, caption=WebSocket Hub Structure, basicstyle=\ttfamily\footnotesize]
type Hub struct {
...
\subsection{Connection Upgrade Process}
Connections begin as HTTP requests upgraded via WebSocket handshake:

\begin{lstlisting}[language=go, caption=WebSocket Connection Upgrade, basicstyle=\ttfamily\footnotesize]
...
The hub manages rooms and broadcasts messages to appropriate clients:

\begin{lstlisting}[language=go, caption=Hub Message Routing, basicstyle=\ttfamily\footnotesize]
func (h *Hub) Run() {
    for {
        select {
...
The gRPC server provides high-performance RPC communication for internal service-to-service calls. It runs on port 9092 and uses Protocol Buffers for efficient binary serialization.

\subsection{Protocol Buffer Definitions}
The service contract is defined in \texttt{proto/manga.proto}:

\begin{lstlisting}[language=protobuf, caption=gRPC Service Definition, basicstyle=\ttfamily\footnotesize]
syntax = "proto3";
package manga;
...
The server implements the service interface and delegates to the service layer:

\begin{lstlisting}[language=go, caption=gRPC Service Implementation, basicstyle=\ttfamily\footnotesize]
type Server struct {
...
Protocol Buffers are compiled to Go code using \texttt{protoc}:

\begin{lstlisting}[language=bash, caption=gRPC Code Generation, basicstyle=\ttfamily\footnotesize]
...
This generates:
\begin{itemize}
    \item \texttt{manga.pb.go} - Message type definitions
    \item \texttt{manga\_grpc.pb.go} - Service interface and client/server stubs
\end{itemize}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[scale=1.2]
        % Draw pyramid layers
        \fill[green!40] (0,0) -- (6,0) -- (5,2) -- (1,2) -- cycle;
        \fill[blue!40] (1,2) -- (5,2) -- (4,3.5) -- (2,3.5) -- cycle;
        \fill[red!40] (2,3.5) -- (4,3.5) -- (3,4.8) -- cycle;

        % Labels - auto-sized to prevent overflow
        \node[align=center, font=\small] at (3,0.8) {\textbf{Unit Tests}\\70\% of tests\\Fast, Isolated};
        \node[align=center, font=\small] at (3,2.6) {\textbf{Integration Tests}\\20\% of tests\\Component Interaction};
        \node[align=center, font=\footnotesize] at (3,4.1) {\textbf{E2E Tests}\\10\% of tests\\Full System};

        % Borders
        \draw[thick] (0,0) -- (6,0) -- (5,2) -- (1,2) -- cycle;
        \draw[thick] (1,2) -- (5,2) -- (4,3.5) -- (2,3.5) -- cycle;
        \draw[thick] (2,3.5) -- (4,3.5) -- (3,4.8) -- cycle;

        % Coverage indicators - repositioned for better alignment
        \node[text width=3cm, align=right, font=\footnotesize] at (-0.8,1) {High Coverage\\Fast Execution};
        \node[text width=3cm, align=right, font=\footnotesize] at (-0.8,4.2) {Complete Flow\\Slower Execution};

        % Optional: Add arrows to connect indicators to pyramid
        \draw[->, dashed, gray] (-0.4,1) -- (0.2,1);
        \draw[->, dashed, gray] (-0.4,4.2) -- (2.2,4.2);
    \end{tikzpicture}
